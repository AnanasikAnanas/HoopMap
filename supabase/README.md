...
для игр и подтверждений, друзей, команд, приглашения, защищённый чат игр, ограничения частоты
запросов и RLS-политики.
...
Функции управления играми также находятся в `bootstrap.sql`. Изменение игры выполняется
атомарно: строка блокируется на время проверки, только организатор или модератор может сохранить
правки, а лимит игроков нельзя уменьшить ниже уже записавшегося количества.
...
При подключённом Telegram организатор получает сообщения о входе и выходе игроков. Участники
получают уведомления о переносе и отмене игры с кнопкой возврата на её страницу. Отправка
уведомления не блокирует успешное изменение игры, если Telegram временно недоступен.
...
проверки, игры, друзей, команды и приглашения. Сама ссылка хранится в Supabase только в виде
SHA-256 хэша, используется один раз и не может объединить Telegram, который уже связан с настоящим
Google/email-аккаунтом.
...
По тому же принципу закрыты `friendships`, `teams`, `team_members` и `game_invitations`.
Социальные данные возвращаются только серверным API после проверки текущей сессии; браузер не
может получить чужие заявки или составы прямым запросом к Supabase Data API.

### Чат игры

У каждой игры есть постоянный чат. Читать его могут только организатор, записавшиеся участники и
модераторы. Сообщения отправляются через серверный API, ограничены 1000 символами и защищены
лимитом частоты. Автор может удалить своё сообщение, а организатор — удалить или закрепить любое.
Удаление очищает текст, но оставляет безопасную системную отметку в истории.

Обновления приходят через Supabase Realtime без отдельного WebSocket-сервера. Таблица
`game_messages` включается в публикацию `supabase_realtime` из `bootstrap.sql`; браузер
подписывается с короткоживущим пользовательским JWT, а RLS повторно проверяет участие в игре.
Если Realtime временно недоступен, интерфейс самостоятельно обновляет сообщения каждые 15 секунд.
...
- страница игры показывает участников, а организатор может изменить время, лимит или отменить её;
- после изменения либо отмены участники с подключённым Telegram получают сообщение;
