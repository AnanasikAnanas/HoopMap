...
create table if not exists public.game_messages (
  ...
  pinned_by bigint references public.profiles(id) on delete set null,
  ...
  check (
    (is_pinned = false and pinned_by is null)
    or (is_pinned = true and pinned_by is not null)
  )
);
create index if not exists game_messages_game_created_idx
  on public.game_messages(game_id, created_at desc);
create index if not exists game_messages_author_idx
  on public.game_messages(author_id);
create index if not exists game_messages_pinned_by_idx
  on public.game_messages(pinned_by)
  where pinned_by is not null;
create index if not exists game_messages_deleted_by_idx
  on public.game_messages(deleted_by)
  where deleted_by is not null;
create unique index if not exists game_messages_one_pin_per_game_idx
  on public.game_messages(game_id)
...
create table if not exists public.friendships (
  id bigint generated by default as identity primary key,
  requester_id bigint not null references public.profiles(id) on delete cascade,
  addressee_id bigint not null references public.profiles(id) on delete cascade,
  status text not null default 'pending'
...
create index if not exists team_members_user_status_idx
  on public.team_members(user_id, status, created_at desc);
...
create table if not exists public.game_invitations (
  id bigint generated by default as identity primary key,
  game_id bigint not null references public.games(id) on delete cascade,
  inviter_id bigint not null references public.profiles(id) on delete cascade,
  invitee_id bigint not null references public.profiles(id) on delete cascade,
...
create index if not exists game_invitations_invitee_status_idx
  on public.game_invitations(invitee_id, status, created_at desc);
create index if not exists game_invitations_inviter_idx
  on public.game_invitations(inviter_id, created_at desc);
create index if not exists game_invitations_team_idx
  on public.game_invitations(team_id)
...
drop policy if exists "game members read chat" on public.game_messages;
create policy "game members read chat"
...
using (
  ...
  or exists (
    ...
    where game.id = game_messages.game_id
      and game.creator_id = public.current_profile_id()
  )
  or exists (
    select 1
    from public.game_participants participant
    where participant.game_id = game_messages.game_id
...
revoke all on table
  public.friendships, public.teams, public.team_members, public.game_invitations
...
grant usage, select on sequence public.game_messages_id_seq to service_role;
...
begin
  if not exists (
    ...
    execute 'alter publication supabase_realtime add table public.game_messages';
  end if;
end;
$$;
...
begin
  ...
  if source_profile_id is not null and source_profile_id <> target_profile_id then
    ...
    update public.game_messages set author_id = target_profile_id
      where author_id = source_profile_id;
    update public.game_messages set pinned_by = target_profile_id
      where pinned_by = source_profile_id;
    update public.game_messages set deleted_by = target_profile_id
      where deleted_by = source_profile_id;

    delete from public.friendships
    where (requester_id = source_profile_id and addressee_id = target_profile_id)
      or (requester_id = target_profile_id and addressee_id = source_profile_id);
    update public.friendships target_friendship
    set status = 'accepted'
    from public.friendships source_friendship
    where source_friendship.status = 'accepted'
      and target_friendship.status = 'pending'
      and target_friendship.id <> source_friendship.id
      and (
        source_friendship.requester_id = source_profile_id
        or source_friendship.addressee_id = source_profile_id
      )
      and least(
        target_friendship.requester_id,
        target_friendship.addressee_id
      ) = least(
        target_profile_id,
        case
          when source_friendship.requester_id = source_profile_id
            then source_friendship.addressee_id
          else source_friendship.requester_id
        end
      )
      and greatest(
        target_friendship.requester_id,
    ...
    delete from public.friendships source_friendship
    where (
      source_friendship.requester_id = source_profile_id
      or source_friendship.addressee_id = source_profile_id
    )
    and exists (
      select 1
      from public.friendships target_friendship
      where target_friendship.id <> source_friendship.id
        and least(
          target_friendship.requester_id,
          target_friendship.addressee_id
        ) = least(
    ...
    set requester_id = target_profile_id
    where requester_id = source_profile_id;
    update public.friendships
    set addressee_id = target_profile_id
    where addressee_id = source_profile_id;

    update public.teams
    set owner_id = target_profile_id
    where owner_id = source_profile_id;
    update public.team_members
    set invited_by = target_profile_id
    where invited_by = source_profile_id;
    ...
    on conflict (team_id, user_id) do update set
      role = case
        when public.team_members.role = 'owner' or excluded.role = 'owner'
          then 'owner'
        when public.team_members.role = 'admin' or excluded.role = 'admin'
          then 'admin'
        else 'member'
      end,
      status = case
        when public.team_members.status = 'active' or excluded.status = 'active'
          then 'active'
        else 'invited'
      end,
      joined_at = case
        when public.team_members.status = 'active'
          then public.team_members.joined_at
        else excluded.joined_at
      end;
    delete from public.team_members where user_id = source_profile_id;

    delete from public.game_invitations
    where (inviter_id = source_profile_id and invitee_id = target_profile_id)
      or (inviter_id = target_profile_id and invitee_id = source_profile_id);
    delete from public.game_invitations source_invitation
    where source_invitation.invitee_id = source_profile_id
      and exists (
        select 1
        from public.game_invitations target_invitation
        where target_invitation.game_id = source_invitation.game_id
          and target_invitation.invitee_id = target_profile_id
      );
    update public.game_invitations
    set invitee_id = target_profile_id
    where invitee_id = source_profile_id;
    update public.game_invitations
    set inviter_id = target_profile_id
    where inviter_id = source_profile_id;
